...
export default {
  ...
  //mixins: [Filterable, Loadable, Themeable],
    ...
    computed: {
        ...
        selected () {
            ...
            for (let index = 0; index < this.value.length; index++) {
                selected.push(this.value[index][this.rowKey])
            }
            return selected
        },
        hasSelectAll () {
    ...
    methods: {
        isSelected (item) {
            return this.selected.includes(item[this.rowKey]);
        },
        hasSelected(val){
            let select = this.selected.slice();
            const find = select.indexOf(val);
            let input=[];
            if(find===-1){
                for(let i=0;i<this.filteredRows.length;i++){
                    if(this.filteredRows[i][this.rowKey]===val){
                        this.value.push(this.filteredRows[i]);
                        input =this.value;
                    }
                }
            }else{
                input=this.value.filter(o=>(o[this.rowKey]!==val));
            }
        ...
        createProps (item, index) {
            ...
            Object.defineProperty(props, 'selected', {
                get: () => { return this.selected.includes(item[this.rowKey])},
                set: value => {
                    if (rowKey == null) {
